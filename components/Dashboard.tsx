...
const EXCHANGE_COLORS: Record<string, string> = {
  Binance: '#F0B90B', Bybit: '#F7A600', OKX: '#00B0FF', Gate: '#00DACC', BingX: '#1DA1F2',
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <span style={{ background: t.direction === 'UP' ? '#00ff8820' : '#ff444420', color: dirColor, border: `1px solid ${dirColor}40`, borderRadius: 6, padding: '2px 10px', fontSize: 12, fontWeight: 700 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 10 }}>
        <div>
          ...
          <div style={{ color: lagColor, fontSize: 28, fontWeight: 800, fontFamily: 'monospace', lineHeight: 1 }}>{t.avgLagSec}с</div>
          <div style={{ color: '#333d52', fontSize: 11, marginTop: 2 }}>min {(t.minLagMs/1000).toFixed(1)}с / max {(t.maxLagMs/1000).toFixed(1)}с</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ color: '#4a5568', fontSize: 10, letterSpacing: 1 }}>ПРИБУТОК 100×</div>
          <div style={{ color: '#a78bfa', fontSize: 22, fontWeight: 700, fontFamily: 'monospace' }}>+{t.profitPct100x.toFixed(1)}%</div>
          <div style={{ color: '#333d52', fontSize: 11 }}>зміна {t.priceChangePct > 0 ? '+' : ''}{t.priceChangePct?.toFixed(3)}%</div>
        </div>
      </div>
      <LagBar ms={t.avgLagMs} max={8000} />
      <div style={{ marginTop: 12, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {Object.entries(t.leadCounts).sort((a, b) => b[1] - a[1]).map(([exch, cnt]) => (
          <span key={exch} style={{ background: `${EXCHANGE_COLORS[exch] || '#888'}18`, border: `1px solid ${EXCHANGE_COLORS[exch] || '#888'}40`, color: EXCHANGE_COLORS[exch] || '#888', borderRadius: 5, padding: '2px 8px', fontSize: 11, fontWeight: 600 }}>
...
export default function Dashboard() {
  ...
  const tokens = (data?.tokens || [])
    ...
    .sort((a, b) => sort === 'lag' ? b.avgLagMs - a.avgLagMs : sort === 'profit' ? b.profitPct100x - a.profitPct100x : b.sampleCount - a.sampleCount)
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '24px 16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <h1 style={{ fontSize: 22, fontWeight: 800, letterSpacing: -0.5 }}>MEXC Lag <span style={{ color: '#3b5bdb' }}>Bot</span></h1>
            </div>
            <div style={{ color: '#2d3748', fontSize: 12, marginTop: 2 }}>Binance · Bybit · OKX · Gate · BingX → MEXC zero-fee</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ width: 8, height: 8, borderRadius: '50%', background: connected ? '#00ff88' : '#ff4444', display: 'inline-block', boxShadow: connected ? '0 0 8px #00ff88' : '0 0 8px #ff4444' }} />
            <span style={{ color: connected ? '#00ff88' : '#ff4444', fontSize: 12 }}>{connected ? 'LIVE' : 'Підключення...'}</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 6, marginLeft: 'auto' }}>
            ...
            {[['lag','За затримкою'],['profit','За прибутком'],['samples','За зразками']].map(([val, label]) => (
              <button key={val} className={`btn ${sort === val ? 'active' : ''}`} onClick={() => setSort(val as any)}>{label}</button>
            ))}
          </div>
        </div>
        ...
        {tokens.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 20px', color: '#2d3748' }}>
            ...
            <div style={{ color: '#374151', fontSize: 16 }}>{data === null ? 'Підключаємось до бірж...' : 'Збираємо дані, очікуйте 30–60 секунд...'}</div>
            <div style={{ color: '#1f2937', fontSize: 13, marginTop: 8 }}>Токен з'явиться тільки якщо MEXC реально відстає від інших бірж</div>
          </div>
        ) : (
          <div className="grid">{tokens.map(t => <TokenCard key={t.symbol} t={t} />)}</div>
        )}

        <div style={{ marginTop: 40, textAlign: 'center', color: '#1f2937', fontSize: 11 }}>
          Оновлення кожні 1.5с · {data?.updatedAt ? new Date(data.updatedAt).toLocaleTimeString('uk-UA') : ''}
        </div>
      </div>
    </>
  )
